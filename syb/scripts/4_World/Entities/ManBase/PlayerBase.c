...
{	
	...
	override void Init()
	{
		...
		RegisterNetSyncVariableBool("IsEncumbered");
		RegisterNetSyncVariableBool("IsRadiated");
	...
	{
		//if (m_sybstats.m_sleepingLevel == SyberiaSleepingLevel.SYBSL_ENERGED) return SyberiaSleepingLevel.SYBSL_COMFORT;
	...
	bool IsFaceBlocked(bool mouthOnly)
	{
		...
		if (itemCheck)
		{
			if (mouthOnly)
			{
				...
				if (itemCheck.IsKindOf("Balaclava3Holes_ColorBase"))
				{
	...
	void SetEncumbered(bool values)
	{
