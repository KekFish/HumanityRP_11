...
{	
	...
	override void DisplayBadge( int key, int value )
	{
		...
		if (key == NTFKEY_FRACTURE || key == NTFKEY_CONCUSSION || key == NTFKEY_ENCUMBRANCE)
		{
			badgetColor = yellowColor;
		}
		else if (key == NTFKEY_BLEEDISH || key == NTFKEY_BULLETHIT || key == NTFKEY_SICK || key == NTFKEY_KNIFEHIT || key == NTFKEY_ZOMBIEBITE || key == NTFKEY_ZVIRUS || key == NTFKEY_CONTAMINATEDAREA)
		{
