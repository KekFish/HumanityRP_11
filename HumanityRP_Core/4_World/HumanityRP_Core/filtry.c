...
{	
	override void Init()
	{
		...
		//INGREDIENTS
		//ingredient 1
		InsertIngredient(0,"TabletsSraczkovix");//you can insert multiple ingredients this way
	...
	override void Do(ItemBase ingredients[], PlayerBase player,array<ItemBase> results, float specialty_weight)//gets called upon recipe's completion
	{
		...
		float fillAmount = filterCurrent + (charcoalCurrent * 25);
		...
		if(fillAmount >= filterMax)
		{
			...
			ingredients[0].SetQuantity(Math.Round(fillAmount / 25));
		}
		else
		{		
